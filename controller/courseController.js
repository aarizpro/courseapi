...
const getCustDetails = asyncHandler(async(req, res) => {
    try {
        const custdetails = await CourseDetails.find({});
...
const createCust = asyncHandler(async(req, res) => {
    try {
        const custdetails = await CourseDetails.create(req.body)
        res.status(200).json(custdetails);
...
const getCustDetail = asyncHandler(async(req, res) =>{
    try {
        const {id} = req.params;
        const custdetails = await CourseDetails.findById(id);
        res.status(200).json(custdetails);
    } catch (error) {
        res.status(500);
        throw new Error(error.message);
...
const updateCust = asyncHandler(async(req, res) => {
    try {
        const {id} = req.params;
        const custdetails = await CourseDetails.findByIdAndUpdate(id, req.body);
        // we cannot find any product in database
        if(!custdetails){
            res.status(404);
            throw new Error(`cannot find any product with ID ${id}`);
        }
        const updatedCust = await CourseDetails.findById(id);
...
const deleteCust = asyncHandler(async(req, res) =>{
    try {
        const {id} = req.params;
        const custdetails = await CourseDetails.findByIdAndDelete(id);
        if(!custdetails){
            res.status(404);
            throw new Error(`cannot find any product with ID ${id}`);
        }
...
const getCoubyField = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const users = await CourseDetails.find(query);
        res.json(users);
    } catch (err) {
