const ClotDetails = require('../models/courseslot')
const asyncHandler = require('express-async-handler')

const getCustDetails = asyncHandler(async(req, res) => {
    try {
        const custdetails = await ClotDetails.find({});
...
const createCust = asyncHandler(async(req, res) => {
    try {
        const custdetails = await ClotDetails.create(req.body)
        res.status(200).json(custdetails);
...
const getCustDetail = asyncHandler(async(req, res) =>{
    try {
        const {id} = req.params;
        const custdetails = await ClotDetails.findById(id);
        res.status(200).json(custdetails);
    } catch (error) {
        res.status(500);
        throw new Error(error.message);
...
const updateCust = asyncHandler(async(req, res) => {
    try {
        const {id} = req.params;
        const custdetails = await ClotDetails.findByIdAndUpdate(id, req.body);
        // we cannot find any product in database
        if(!custdetails){
            res.status(404);
            throw new Error(`cannot find any product with ID ${id}`);
        }
        const updatedCust = await ClotDetails.findById(id);
...
const deleteCust = asyncHandler(async(req, res) =>{
    try {
        const {id} = req.params;
        const custdetails = await ClotDetails.findByIdAndDelete(id);
        if(!custdetails){
            res.status(404);
            throw new Error(`cannot find any product with ID ${id}`);
        }
...
const getCoubyField = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const users = await ClotDetails.find(query);
        res.json(users);
    } catch (err) {
